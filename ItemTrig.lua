...
function ItemTrig.questTest(filter)
   local count = GetNumJournalQuests()
   for i = 1, count do
      local quest = ItemTrig.QuestInterface:new(i)
      local echo = true
      if filter and not (filter:lower() == (quest.name or ""):lower()) then
         echo = false
      end
      if echo then
         d("Quest #" .. i .. " is: " .. tostring(quest.name) .. ":")
         d(quest)
      end
   end
end
function ItemTrig.covetousTest(itemName)
   local quest
   do
      local count = GetNumJournalQuests()
      for i = 1, count do
         local name = GetJournalQuestName(i)
         if name:lower() == "the covetous countess" then
            quest = ItemTrig.QuestInterface:new(i)
            break
         end
      end
      if not quest then
   ...
   local item
   do
      ItemTrig.forEachBagSlot(BAG_BACKPACK, function(interface)
         if interface.formattedName:lower() == itemName:lower() then
            item = interface
            return true
         end
      end)
   ...
   d("Quest steps: " .. #quest.steps)
   quest:forEachStep(function(step)
      d("Step " .. step.index .. " with " .. #step.conditions .. " conditions:")
      d(LocalizeString("|u20:0::TEXT=|u<<1>>", step.text))
      d(LocalizeString("|u20:0::SHORT=|u<<1>>", step.textShort))
      step:forEachCondition(function(condition)
         if condition:isFulfilledByItem(item) then
            d("|u40:0::IS FULFILLED BY ITEM!|u")
         else
            d("|u40:0::Not fulfilled by the item.|u")
         end
      end)
   end)
end